...
Adapted from the production codebase for Python >=3.9 compatibility.
...
# ============================================================
PQ_M1: float = 2610.0 / 16384.0  # 0.1593017578125
PQ_M2: float = 2523.0 / 4096.0 * 128.0  # 78.84375
PQ_C1: float = 3424.0 / 4096.0  # 0.8359375
PQ_C2: float = 2413.0 / 4096.0 * 32.0  # 18.8515625
PQ_C3: float = 2392.0 / 4096.0 * 32.0  # 18.6875
PQ_PEAK_LUMINANCE: float = 10000.0  # cd/m^2


def pq_eotf(signal: NDArray[np.floating]) -> NDArray[np.floating]:
    """PQ EOTF: signal [0,1] -> linear luminance [0, 10000] cd/m^2.

    Converts PQ-encoded signal values to absolute linear luminance.
    """
    signal = np.clip(signal, 0.0, 1.0)
    vp = np.power(signal, 1.0 / PQ_M2)
    num = np.maximum(vp - PQ_C1, 0.0)
    den = PQ_C2 - PQ_C3 * vp
    den = np.maximum(den, 1e-12)
    linear = np.power(num / den, 1.0 / PQ_M1)
    return linear * PQ_PEAK_LUMINANCE


def pq_oetf(luminance: NDArray[np.floating]) -> NDArray[np.floating]:
    """PQ inverse EOTF (OETF): linear luminance [0, 10000] cd/m^2 -> signal [0, 1].

    Converts absolute linear luminance to PQ-encoded signal values.
    """
    luminance = np.clip(luminance, 0.0, PQ_PEAK_LUMINANCE)
    y = luminance / PQ_PEAK_LUMINANCE
    ym1 = np.power(y, PQ_M1)
    num = PQ_C1 + PQ_C2 * ym1
    den = 1.0 + PQ_C3 * ym1
    signal = np.power(num / den, PQ_M2)
    return signal
...
HLG_B: float = 1.0 - 4.0 * HLG_A  # 0.28466892
HLG_C: float = 0.5 - HLG_A * np.log(4.0 * HLG_A)  # 0.55991073
...
def linearize(
    ...
    Args:
        ...
        transfer: Transfer function identifier ("pq", "hlg", "bt1886", "gamma24").
